...
class Aleph(TemplateLearner):
    ...
    def learn(
        ...
        while len(pos) > 0 and not stop:
            ...
            extensions = [
                lambda x, y=pred: plain_extension(x, y, connected_clauses=True)
                for pred in body_predicates
            ]
            instantiations = [
                lambda x, y=const: variable_instantiation(x, y) for const in constants
            ]

            hs = TopDownHypothesisSpace(
                primitives=extensions + instantiations,
