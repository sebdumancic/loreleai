...
num_trials = 10
num_tasks = 1000
playtimes = [200, 400, 600, 800, 1000]  # list(range(0,2200,200))
...
def build_p():
    ...
    for k in trials:
        for p in playtimes:
            playf = f"{ROOT_FILE}/refactored_programs_p/robots-6x6/{SETUP}/refactored_programs-{p}-{k}.pl" # "'data/play-{}-{}'".format(p,k)
            buildf = f"'{ROOT_FILE}/build/robots-6x6/build-{k}.pl'"   # "'data/build-{}'".format(k)
...
def build_pb():
    ...
    for k in trials:
        for p in playtimes:
            playf = f"{ROOT_FILE}/refactored_programs_pb/robots-6x6/refactored_programs-{p}-{k}.pl" # "'data/play-{}-{}'".format(p,k)
            buildf = f"'{ROOT_FILE}/build/robots-6x6/build-{k}.pl'"   # "'data/build-{}'".format(k)
...
def test_new():
    ...
    for k in trials:
        for p in playtimes:
            buildf = f"'{ROOT_FILE}/build/robots-6x6/build-{k}.pl'"
